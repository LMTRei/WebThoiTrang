...
namespace WebThoiTrang.Areas.Admin.Controllers
{
    public class DonHangsController : BaseController
    {
        ...
        public ActionResult Index(string searchString, string currenFilter, int? page)
        {
            ...
            try
            {
                var donHangs = db.DonHang.Select(t => t);
        ...
        {
            ViewBag.MaTK = new SelectList(db.TaiKhoan, "MaTK", "DiaChiEmail");
            return View();
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create([Bind(Include = "SoDH,NgayDat,DiaChiGH,GhiChu,TongTien,MaTK,TinhTrang,HinhThucTT")] DonHang donHang)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        ViewBag.Error = check;
                        ViewBag.MaTK = new SelectList(db.TaiKhoan, "MaTK", "DiaChiEmail", donHang.MaTK);
                        return View(donHang);
                    }
        ...
        public void updateSoLuongSP(int SoDH)
        {
            ...
            chiTietDHs = db.ChiTietDH.Where(c => c.SoDH == SoDH).ToList();
            if (chiTietDHs.Count > 0)
            {
                foreach (ChiTietDH c in chiTietDHs)
                {
                    SanPham sanPham = db.SanPham.FirstOrDefault(s => s.MaSP == c.MaSP);
                    if (sanPham != null)
                    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "SoDH,NgayDat,DiaChiGH,GhiChu,TongTien,MaTK,TinhTrang,HinhThucTT")] DonHang donHang)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            try
            {
                chiTietDonHangs = db.ChiTietDH.Where(ct => ct.SoDH == id).ToList();
                if (chiTietDonHangs.Count > 0)
                {
                    foreach (var ct in chiTietDonHangs)
                    {
                        db.ChiTietDH.Remove(ct);
                    }
        ...
        public ActionResult ChiTietDonHang(int? id)
        {
            ...
            try
            {
                chiTietDHs = db.ChiTietDH.Where(ct => ct.SoDH == id).ToList();
            }
            catch (Exception e)
            {
        ...
        public ActionResult CreateChiTietDonHang([Bind(Include = "SoDH,MaSP,SoLuong")] ChiTietDH chiTietDH)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        ViewBag.Error = check;
                        ViewBag.MaSP = new SelectList(db.SanPham, "MaSP", "TenSP", chiTietDH.MaSP);
        ...
        public ActionResult EditChiTietDonHang(int? id, int? SoDH)
        {
            ...
            try
            {
                chiTietDH = db.ChiTietDH.Find(id);
                if (chiTietDH == null)
                {
                    return RedirectToAction("NotFoundPage", "ErrorAdmin");
                }
        ...
        public ActionResult DeleteChiTietDonHang(int id, int SoDH)
        {
            try
            {
                var chitietDH = db.ChiTietDH.Find(id);
                if (chitietDH == null)
                {
                    return RedirectToAction("NotFoundPage", "ErrorAdmin");
                }
                db.ChiTietDH.Remove(chitietDH);
                db.SaveChanges();
        ...
        {
            List<ChiTietDH> chiTietDHs = new List<ChiTietDH>();
            chiTietDHs = db.ChiTietDH.Where(ct => ct.SoDH == SoDH).ToList();
            int total = 0;
            if (chiTietDHs.Count > 0)
            {
                foreach (ChiTietDH c in chiTietDHs)
                {
                    SanPham sanPham = (SanPham)db.SanPham.FirstOrDefault(sp => sp.MaSP == c.MaSP);
                    if (sanPham != null)
                    {
            ...
            DonHang donHang = db.DonHang.Find(SoDH);
            if (donHang != null)
            {
