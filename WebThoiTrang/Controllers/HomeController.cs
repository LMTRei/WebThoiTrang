    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            try
            {
                ...
                sanPhams = db.SanPham.Where(s => s.SoLuong > 0).OrderByDescending(s => s.NgayTao).Take(8).ToList();
            }
            catch (Exception e)
            {
        ...
        public ActionResult ProductDetails(int? id)
        {
            ...
            try
            {
                ...
                sanPham = db.SanPham.SingleOrDefault(sp => sp.MaSP == id);
            }
            catch (Exception e)
            {
                return RedirectToAction("ServerError", "Error");
        ...
        public ActionResult Login(string DiaChiEmail, string MatKhau)
        {
            ...
            else
            {
                if (ModelState.IsValid)
                {
                    ...
                    try
                    {
                        var user = db.TaiKhoan.SingleOrDefault(u => u.DiaChiEmail.Equals(DiaChiEmail) && u.MatKhau.Equals(passhash));
                        if (user != null)
                        {
                            if (user.TrangThai == 0)
                            {
        ...
        public ActionResult Register([Bind(Include = "MaTK, DiaChiEmail, HoTen, MatKhau, VaiTro, GioiTinh, NgaySinh, SoDienThoai")] TaiKhoan taiKhoan)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    var user = db.TaiKhoan.SingleOrDefault(u => u.DiaChiEmail.Equals(taiKhoan.DiaChiEmail) || u.SoDienThoai.Equals(taiKhoan.SoDienThoai));
                    if (user == null)
                    {
                        //TẠO MỚI TÀI KHOẢN
        ...
        public List<CartItem> GetListCartItem(int MaTK)
        {
            ...
            try
            {
                carts = db.Cart.Where(c => c.MaTK == MaTK).ToList();

                for (int i = 0; i < carts.Count; i++)
                {
                    int MaSP = carts[i].MaSP;
                    SanPham sanPham = db.SanPham.FirstOrDefault(sp => sp.MaSP == MaSP);
        ...
        public ActionResult Update_Cart_quantity(FormCollection form)
        {
            ...
            try
            {
                Cart cart = db.Cart.FirstOrDefault(sp => sp.MaSP == productID);
        ...
        public ActionResult AddItem_icon(int productID)
        {
            ...
            try
            {
                product = db.SanPham.SingleOrDefault(s => s.MaSP == productID);
                int MaTK = Convert.ToInt32(Session["MaTK"]);
                if (product != null && product.SoLuong > 0)
                {
           
                    Cart cart = new Cart();
                    cart = db.Cart.Where(c => c.MaTK == MaTK).Where(c => c.MaSP == productID).FirstOrDefault();
                    if (cart == null)
                    {
                        Cart c = new Cart();
                        c.MaTK = MaTK;
        ...
        public ActionResult Remove_Cart(int productID)
        {
            try
            {
                Cart cart = db.Cart.FirstOrDefault(c => c.MaSP == productID);
                if (cart == null)
                {
                    return RedirectToAction("ShopCart", "Home");
                }
        ...
        {
            var taiKhoan = db.TaiKhoan.Find(MaTK);
        ...
        public ActionResult DanhsachDonhang()
        {
            ...
            try
            {
                donHangs = db.DonHang.Where(dh => dh.MaTK == MaTK).ToList();
            }
            catch (Exception e)
            {
                return RedirectToAction("ServerError", "Error");
        ...
        public ActionResult HuyDon(int id)
        {
            ...
            try
            {
                donHang = db.DonHang.FirstOrDefault(dh => dh.SoDH == id);
                if (donHang != null)
                {
                    if (donHang.TinhTrang != 2)
                    {
                        donHang.TinhTrang = 3;
                        db.DonHang.AddOrUpdate(donHang);
                        db.SaveChanges();
                    }
                    else
                    {
        ...
        public ActionResult ChiTietDon(int? id)
        {
            ...
            try
            {
                chiTietDH = db.ChiTietDH.Where(c => c.SoDH == id).ToList();
                if (chiTietDH != null)
                {
        ...
        //MENU SHOP
        public ActionResult Shop(int? page, int? MaLoai, String searchString)
        {
            List<SanPham> sanPhams = new List<SanPham>();
            ...
            try
            {
                if (MaLoai == null)
                {
                    sanPhams = db.SanPham.OrderByDescending(s => s.NgayTao).Select(s => s).ToList();
                }
                else if (MaLoai != null)
                {
                    sanPhams = db.SanPham.Where(s => s.MaLoai == MaLoai).OrderByDescending(s => s.NgayTao).Select(s => s).ToList();
                }
        ...
        {
            var danhMucs = db.LoaiSP.Select(s => s);
        ...
        public ActionResult Contact()
        {
            ...
            try
            {
                lienHe = db.LienHe.FirstOrDefault();
            }
            catch (Exception e)
            {
                return RedirectToAction("ServerError", "Error");
        ...
        public ActionResult New()
        {
            ...
            try
            {
                sanPhams = db.SanPham.OrderByDescending(s => s.PhanTramKM).Take(5).ToList();
            }
            catch
            {
                return RedirectToAction("ServerError", "Error");
        ...
        {
            var sanPhamLienQuans = db.SanPham.Where(s => s.MaLoai == MaDM && s.SoLuong > 0).Select(s => s);

            return PartialView(sanPhamLienQuans);
        }

        //MENU SHOP
        public ActionResult ShopW(int? page, int? MaLoai, String searchString)
        {
            List<SanPham> sanPhams = new List<SanPham>();
            ...
            try
            {
                ...
                {
                    sanPhams = db.SanPham.Where(s => s.LoaiSP.MaDM == 2 && s.MaLoai == MaLoai)
        ...
        public ActionResult SanPhamDaBanNhieuNhat()
        {
            var sanPhams = db.SanPham
                .Join(db.ChiTietDH, sp => sp.MaSP, ctdh => ctdh.MaSP, (sp, ctdh) => new { SanPham = sp, ChiTietDonHang = ctdh })
                .GroupBy(x => x.SanPham)
                .Select(x => new { SanPham = x.Key, SoLuong = x.Sum(y => y.ChiTietDonHang.SoLuong) })
                .OrderByDescending(x => x.SoLuong)
                .Select(x => new { x.SanPham.MaSP, x.SanPham.TenSP, x.SoLuong })
                .Take(3) // Lấy 3 sản phẩm được mua nhiều nhất
                .ToList();

            ViewBag.Column = "SoLuong"; // Sắp xếp theo cột SoLuong
